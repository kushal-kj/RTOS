...
const osMessageQueueAttr_t msgQueueAtrribs = {
		.name="Msg1 Queue"
};


osEventFlagsId_t EventFlag_LED;
static const osEventFlagsAttr_t EventFlagAtrr_LED={
		.name="LED_Events"
};
...
int main(void)
{
  ...
  EventFlag_LED = osEventFlagsNew(&EventFlagAtrr_LED);		//Create a event flag
  ...
  msg_queue_id=osMessageQueueNew(5, sizeof(uint16_t), &msgQueueAtrribs);		//Create a Message Queue
