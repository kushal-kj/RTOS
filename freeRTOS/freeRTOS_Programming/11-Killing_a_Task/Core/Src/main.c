...
uint32_t execution_monitor;		//Counter variable for killing the task
uint32_t resume_monitor;		//Counter variable for resuming the task
bool is_killed=false;		//Flag
...
void vGreenLEDControllerTask(void *pvParameters)		//Task2 Function
{
	...
	while(1)
	{
		...
		if(execution_monitor>=50)
		{
			is_killed=true;
			execution_monitor=0;
			vTaskDelete(NULL);			//Killing the Task2 itself
		}
	}
...
void vYellowLEDControllerTask(void *pvParameters)		//Task3 Function
{
	...
	while(1)
	{
		...
		if(is_killed)
		{
			...
			{
				vTaskResume(green_Handle);		//Resuming the Task2
				resume_monitor=0;
				is_killed=false;
			}
		}
